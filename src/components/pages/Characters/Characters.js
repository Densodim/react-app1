...
import { add, bioSelector} from "../../../store/cardSlice";
...
const Characters = () => {
  // const [chapter, setChapter] = useState(CHARACTER); // стейт получения карточек персонажей

  const chapterIdSelector = useSelector(cardSelector); // селектор получения карточек персонажей
  // console.log(chapterIdSelector);
  const dispatch = useDispatch(); // диспатч получения карточек персонажей

  const {CHARACTER} = useSelector(bioSelector); // селектор получения карточек персонажей
  ...
  const handleLikeFilter = () => {
    dispatch(add(
      CHARACTER.filter((item) =>  chapterIdSelector[item.id])
    ));
    navigation('/like');
  };
  ...
  return (
    <>
      <div className={style.filter}>
        Filter
        <div onClick={handleLikeFilter} className={style.filterLike}>Like active</div>
      </div>
